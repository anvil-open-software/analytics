 ...
 */
package com.dematic.labs.analytics.ingestion.spark.drivers.event.wordcount;
...
public final class JavaKinesisWordCountASL { // needs to be public for access from run-example
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 4) {
            System.err.println(
                    "Usage: JavaKinesisWordCountASL <stream-name> <endpoint-url>\n\n" +
                            ...
                            "                   (e.g. https://kinesis.us-east-1.amazonaws.com)\n" +
                            "    <batch interval> in milliseconds i.e. 5000 for 5s\n"+
        ...
        Duration batchInterval =  new Duration(Integer.valueOf(args[3]));


        // we're passing in the checkpoint dir for convence
        String checkpointDir = System.getProperty(DriverConsts.SPARK_CHECKPOINT_DIR);
        ...
        for (int i = 0; i < numStreams; i++) {
            streamsList.add(
                    KinesisUtils.createStream(jssc, kinesisAppName, streamName, endpointUrl, regionName,
                            InitialPositionInStream.TRIM_HORIZON, kinesisCheckpointInterval, StorageLevel.MEMORY_ONLY())
            );
        }
