	...
	describe('StringToSign Service', function() {
		...
		it('should set up the httpverb',
			inject(['StringToSign', function(StringToSign) {
				...
				expect(expected).toBe(StringToSign.getHttpVerb());
			}])
		);
		it('should set up the CannonicalHeaders - without x-dlabs-date',
			inject(['StringToSign', function(StringToSign) {
				...
				StringToSign.setCannonicalHeaders(config);
				expect(expected).toBe(StringToSign.getCannonicalHeaders());
			}])
		);
		...
		it('should set up the DLabsHeaders without x-lab-date',
			inject(['StringToSign', function(StringToSign) {
				...
				StringToSign.setDlabHeaders(config);
				expect(expected).toBe(StringToSign.getDlabHeaders());
			}])
		);
		...
		it('should set up the uri',
			inject(['StringToSign', function(StringToSign) {
				...
				StringToSign.setUri(config);
				expect(expected).toBe(StringToSign.getUri());
			}])
		);
		it('should set up the query parameters', function() {
			inject(['StringToSign', function(StringToSign) {
				...
				StringToSign.setQueryParameters(config);
		...
		it('should set up the string to sign', function() {
			inject(['StringToSign', function(StringToSign){
				...
				stringToSign = StringToSign.buildStringToSign(config);
				expect(expected).toBe(stringToSign);
			}])
		});
	})
});
